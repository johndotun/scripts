...
if ($i == 1) then
mpirun -n $NPROCS $chmexe move=$i trajno=$trajno seed=$ISEED -i grp94-close.strt.inp >& grp94-close.traj$trajno.move$i.out &
wait
...
mpirun -n $NPROCS $chmexe move=$i trajno=$trajno -i grp94-open.inp >& grp94-open.traj$trajno.move$i.out &
wait
...
mpirun -n $NPROCS $chmexe move=$i trajno=$trajno -i grp94-close.restrt.inp >& grp94-close.traj$trajno.move$i.out &
wait
cp tmd-pep-grp94-close.traj$trajno.move$i.cor tmd-pep-grp94-close.crd
cp tmd-pep-grp94-close.traj$trajno.move$i.res tmd-pep-grp94-close.rea
...
## ie
## echo "$chmexe move=$i trajno=$trajno seed=$ISEED -i grp94-close.traj$trajno.move$i.out" >> debug
